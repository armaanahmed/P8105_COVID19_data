...
mh_care_df = read.csv("Mental_Health_Care_in_the_Last_4_Weeks.csv") %>%  
  janitor::clean_names() %>% 
  select(i_indicator, group, state, subgroup, time_period_label,value)

head_mh = mh_care_df %>% select(i_indicator, state, subgroup, time_period_label,value) %>% knitr::kable() 
